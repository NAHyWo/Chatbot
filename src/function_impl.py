...
#그래프 그리는 함수
def plot_daily_completed_quantity(start_date: Optional[str] = None, end_date: Optional[str] = None):
    if df.empty:
        print("생산 데이터가 없습니다.")
        return
    ...
    for day in pd.date_range(start, end):
        ...
        if not day_df.empty:
            last_completed = day_df.sort_values('timestamp').iloc[-1]['completed_qty']
            daily_completed[day_str] = int(last_completed)

    if not daily_completed:
        return {"error": "해당 기간에 완료된 작업이 없습니다."}

    # 그래프 그리기
    dates = list(daily_completed.keys())
    values = list(daily_completed.values())

    plt.figure(figsize=(10, 5))
    plt.plot(dates, values, marker='o')
    plt.title(f"일별 완료 수량 ({start_date} ~ {end_date})")
    plt.xlabel("날짜")
    plt.ylabel("완료 수량")
